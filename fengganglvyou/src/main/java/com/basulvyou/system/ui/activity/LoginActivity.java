...
public class LoginActivity extends BaseActivity implements View.OnClickListener, PlatformActionListener {
    ...
    private void intiView() {
        ...
        boolean is_sel_login = MySharepreference.getInstance(this).getBoolean(MySharepreference.IS_SEL_LOGIN, false);
        forgetPwd.getPaint().setFlags(Paint.UNDERLINE_TEXT_FLAG);
        check_login = (CheckBox) findViewById(R.id.check_login);
        if (is_sel_login) {
            String name = MySharepreference.getInstance(this).getString(MySharepreference.LOGIN_NAME);
            String pwd = MySharepreference.getInstance(this).getString(MySharepreference.LOGIN_PWD);
            editPhone.setText(name);
            editPwd.setText(pwd);
            check_login.setChecked(is_sel_login);
        }
    ...
    private void LoginHander(String json) {
        ...
        if (userEntity != null) {
            ...
            if (check_login.isChecked()) {
                if (!StringUtil.isEmpty(editPhone.getText().toString())) {
                    MySharepreference.getInstance(this).putString(MySharepreference.LOGIN_NAME, editPhone.getText().toString());
                    MySharepreference.getInstance(this).putString(MySharepreference.LOGIN_PWD, editPwd.getText().toString());
                }
            }
            MySharepreference.getInstance(this).putBoolean(MySharepreference.IS_SEL_LOGIN, check_login.isChecked());
            MySharepreference.getInstance(this).putString(MySharepreference.USER_PHOTO, userEntity.user_logo);
        }
