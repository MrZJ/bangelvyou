...
public class CarpoolingAdapter extends AbsBaseAdapter<CarpoolingInfoEntity> {
    ...
    public CarpoolingAdapter(List<CarpoolingInfoEntity> branchInfos, Context context, int fragmetPos, String resType) {
        mData = branchInfos;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (viewEntity != null) {
            ...
            } else if (fragmetPos == 2) {
                if (viewEntity.comment_by_self_count > 0) {
                    holder.tv_info_gocar.setText("已评价");
                } else {
                    holder.tv_info_gocar.setText("待评价");
                }
            } else {
                if (viewEntity.comment_by_self_count > 0) {
                    holder.tv_info_gocar.setText("已评价");
            ...
            try {
                if (!StringUtil.isEmpty(viewEntity.credit)) {
                    float score = 0;
                    float rating = 0;
