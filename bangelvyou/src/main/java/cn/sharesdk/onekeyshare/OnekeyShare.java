...
public class OnekeyShare implements PlatformActionListener, Callback {
	...
	private boolean shareSinaWeibo;
	...
	public void share(HashMap<Platform, HashMap<String, Object>> shareData) {
		...
		for (Entry<Platform, HashMap<String, Object>> ent : shareData.entrySet()) {
			...
			if (isSinaWeibo && plat.isClientValid()) {
				shareSinaWeibo = true;
				Object title = data.get("title");
//				Object download = data.get("url");
				if (title != null && !TextUtils.isEmpty(String.valueOf(title))){
//					data.put("text", title.toString() + download.toString());
					data.put("text", title.toString());
				}
			}
