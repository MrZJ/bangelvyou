...
public class NewGoodFragment extends BaseFragment implements View.OnKeyListener, View.OnClickListener {
...
//        setTopTitle("新品");
    ...
    private void setData() {
        ...
        Log.e("HTTP_MARKET_URL", ConfigUtil.HTTP_NEW_GOOD_URL);
        webView.loadUrl(ConfigUtil.HTTP_NEW_GOOD_URL);
        webView.setWebChromeClient(new MyWebChromeClient(webPro));
        webView.setWebViewClient(new MyWebViewClient(titleMap, ConfigUtil.HTTP_NEW_GOOD_URL, this));
    ...
    public boolean onKey(View view, int keyCode, KeyEvent event) {
        if (event.getAction() == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_BACK && webView.canGoBack()) {
            if (!(ConfigUtil.HTTP_NEW_GOOD_URL).equals(webView.getUrl())) {
                //表示按返回键时的操作
                webView.goBack();   //后退
                return true;
            }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_top_goback://返回
                if (webView.canGoBack() && !(ConfigUtil.HTTP_NEW_GOOD_URL).equals(webView.getUrl())) {  //表示按返回键时的操作
                    webView.goBack();
                }
                break;
        }
